...
Known product defects and characterization gaps are tracked in [KNOWN_ISSUES.md](KNOWN_ISSUES.md).
...
npm run test:core                # API + Chromium smoke + UI regression
npm run test:api
npm run test:ui
npm run test:integration
npm run test:known-defects       # expected product failures in a separate lane
npm test                         # 35-scenario Chromium regression
...
| Layer       | Unique scenarios | Default regression | Purpose                                      |
| ----------- | ---------------: | -----------------: | -------------------------------------------- |
| Unit        |               36 |                 36 | Framework logic without remote credentials   |
| API         |               15 |                 13 | Contracts, authorization, RBAC and resources |
| Smoke       |                6 |                  6 | Critical public, user and admin entry points |
| UI          |               10 |                  9 | Browser-owned behaviour                      |
| Integration |                7 |                  7 | Cross-layer persistence and observability    |

The remote inventory contains 38 unique scenarios. The default Chromium regression runs 35 and excludes three expected failures tracked in [KNOWN_ISSUES.md](KNOWN_ISSUES.md). Cross-browser smoke executes the same six smoke scenarios in Chromium, Firefox, and WebKit, producing 18 browser executions. Consequently, `npm run test:list` reports 50 Playwright executions across all configured projects: 32 non-smoke scenarios plus 18 cross-browser smoke executions.
...
The access key is never generated by CI. Rotation is manual: submit the vacancy form once, save every issued value immediately, update `.env` and the protected GitHub Environment `qa`, then invalidate the previous credentials if supported.
...
| ---------------------------------- | ------------------ | ----------------------------------------------------------------------- |
| Push to any branch                 | `Framework checks` | Lint, TypeScript typecheck and 36 unit tests                            |
| Pull request to `main`             | `Quality gate`     | Format, lint, types, unit coverage and secret-free Playwright discovery |
| Push to `main`                     | `Remote E2E`       | Six smoke scenarios in Chromium, Firefox and WebKit                     |
| Manual `workflow_dispatch`         | `Remote E2E`       | Selected `smoke`, `api`, `ui`, `integration`, `known-defects` or `all`  |
| `qa-environment-deployed` dispatch | `Remote E2E`       | Complete 35-scenario regression                                         |

The `all` target runs API + Chromium smoke + UI first, then admin/analytics, profile/todo lifecycle, and pagination integration groups serially on fresh hosted runners. Manual `integration` uses the same three groups. The shared QA environment never receives concurrent mutation suites, and there is no nightly schedule. Dependency upgrades are reviewed and applied manually.
