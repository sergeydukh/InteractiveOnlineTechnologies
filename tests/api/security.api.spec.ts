...
test.describe('API security', { tag: '@api' }, () => {
  test('requires X-Access-Key before evaluating auth credentials', async ({ apiVariants }) => {
    ...
    expect(missing).toMatchObject({
      ok: false,
      status: 401,
      error: { message: 'Missing X-Access-Key header' },
    });
    expect(malformed).toMatchObject({ ok: false, status: 401, error: { message: 'Invalid access key format' } });
  });
  ...
  test('requires a bearer session for user and admin resources', async ({ api }) => {
    ...
    for (const result of results) {
      expect(result).toMatchObject({ ok: false, status: 401, error: { message: 'Access denied' } });
    }
  });
  ...
  test('requires access key and Basic Auth together for analytics', async ({ apiVariants }) => {
    ...
    expect(missingAccessKey).toMatchObject({
      ok: false,
      status: 401,
      error: { message: 'Missing X-Access-Key header' },
    });
    expect(missingBasic).toMatchObject({
      ok: false,
      status: 401,
